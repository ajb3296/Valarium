...
from bot import LOGGER, BOT_VER, color_code, DebugServer
...
class Owners(commands.Cog):
    ...
    async def serverlist(self, ctx):
        ...
        if len(self.bot.guilds) <= page:
            embed = discord.Embed(title = f"{self.bot.user.name} (이)가 들어가 있는 서버목록", description=f"**{len(self.bot.guilds)}개**의 서버", color=color_code)
            srvr = str()
        ...
        for i in range(1, allpage + 1):
            ...
            pages_list.append(
                [
                    discord.Embed(title = f"{self.bot.user.name} (이)가 들어가 있는 서버목록", description=f"**{len(self.bot.guilds)}개**의 서버\n\n{srvr}")
                ]
            )
